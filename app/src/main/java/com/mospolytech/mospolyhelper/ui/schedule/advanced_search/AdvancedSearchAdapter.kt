...
class AdvancedSearchAdapter(var filter: AdvancedSearchFilter)
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.checkBox.isChecked = filter.getChecked(dataSet[position])
    }
...
class AdvancedFilter(
    ...
    init {   // TODO Right here
        this.normalized = ArrayList(originDataSet.size)
        for (i in normalized.indices) {
            normalized[i] = String(
                originDataSet[i].filter { it.isLetterOrDigit() }.toCharArray()
            )
        }

        checkedArray = ArrayList(originDataSet.size)
    ...
    override fun getChecked(index: Int) = checkedArray[index]
    ...
    override fun getFiltered(template: String): List<Int> {
        if (template.isEmpty() || originDataSet.isEmpty()) {
            val array = ArrayList<Int>(originDataSet.size)
            var j = 0
        ...
        for (i in originDataSet.indices) {
            if (templateRegex.containsMatchIn(this.normalized[i])) {
                newList.add(i)
                if (getChecked(newList[newList.lastIndex])) {
                    var buf = newList[j]
...
class SimpleFilter(
    ...
    init {
        checkedArray = ArrayList<Boolean>(originDataSet.size)
        checkedIndexes = checkedList
        for (value in checkedList) {
            checkedArray[value] = true
        }
