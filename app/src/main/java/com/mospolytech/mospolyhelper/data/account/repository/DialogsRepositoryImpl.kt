...
class DialogsRepositoryImpl(
    ...
    override suspend fun getDialogs(emitLocal: Boolean) = flow {
        emit(Result0.Loading)
        if (emitLocal) {
            prefDataSource.getResultObject<List<DialogModel>>(PreferenceKeys.Dialogs)?.let {
                emit(it)
                emit(Result0.Loading)
            }
        }
        val res = api.getDialogs()
            .onSuccess {
